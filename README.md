...
| 场景 | 去哪 |
|------|------|
| 完全没基础，想系统学 | [Web3 百科全书](encyclopedia/README.md) → 卷一开始顺读 |
| 日报/群里看到不懂的词 | [百科附录A 黑话词典](encyclopedia/README.md#附录) 或 [基础知识](#一基础知识-basics) |
...
## 零、Web3 百科全书 `encyclopedia/` 🆕

> **从零读到懂的主干读物**。七卷：底层原理 → 金融机制 → 赛道全景 → 项目档案 → 人物志 → 事故档案 → 实务自保。
> 全书每条事实带来源分级标记（A/B/C/传闻/已证伪/推测），传闻单独归档。
>
> 👉 **[目录与编写规范](encyclopedia/README.md)** ｜ 状态：目录已定，内容分批填充

和其他部分的分工：百科是主干（是什么/有哪些/谁在做），`methods/` 是深水区（怎么判断值不值），`cases/` 是样本，`research/` 是时效档案。百科不重复写，直接链过去。
